const rough = function () {
    ...
    const b = Math.tan, c = Math.pow, d = Math.cos, e = Math.sin, f = Math.PI, g = Math.sqrt, h = Math.max,
        j = Math.min,
        i = Math.abs, k = Number.MAX_VALUE;
    ...
    class l {
        ...
        compare(d) {
            ...
            let e = k, f = k, g = 0, l = 0, m = this.a, n = this.b, b = this.c;
    ...
    class m {
        ...
        getNextLine() {
            if (1e-4 > i(this.sinAngle)) {
                ...
                let b = this.pos - this.deltaX / 2, c = this.pos + this.deltaX / 2, d = this.bottom, e = this.top;
                if (this.pos < this.right + this.deltaX) {
                    for (; b < this.left && c < this.left || b > this.right && c > this.right;) if (this.pos += this.hGap, b = this.pos - this.deltaX / 2, c = this.pos + this.deltaX / 2, this.pos > this.right + this.deltaX) return null;
                    let f = new l(b, d, c, e);
                    f.compare(this.sLeft) == a().INTERSECTS && (b = f.xi, d = f.yi), f.compare(this.sRight) == a().INTERSECTS && (c = f.xi, e = f.yi), 0 < this.tanAngle && (b = this.right - (b - this.left), c = this.right - (c - this.left));
                    let g = [b, d, c, e];
    ...
    class o {
        ...
        get closed() {
            if ("undefined" == typeof this._closed) {
                this._closed = !1;
                for (let a of this.segments) "z" === a.key.toLowerCase() && (this._closed = !0)
            }
            return this._closed
        }
        ...
        processPoints() {
            ...
            for (let c, d = 0; d < this.segments.length; d++) {
                ...
                ("m" === c.key || "M" === c.key) && (a = null), c.point && (b = c.point, !a && (a = c.point)), ("z" === c.key || "Z" === c.key) && (a = null)
            }
        }

        parseData(a) {
            const b = this.tokenize(a);
            let c = 0, d = b[c], e = "BOD";
            for (this.segments = []; !d.isType(this.EOD);) {
                let f;
                const g = [];
                if (!("BOD" == e)) d.isType(this.NUMBER) ? f = this.PARAMS[e].length : (c++, f = this.PARAMS[d.text].length, e = d.text); else if ("M" == d.text || "m" == d.text) c++, f = this.PARAMS[d.text].length, e = d.text; else return this.parseData("M0,0" + a);
                if (c + f < b.length) {
                    let h, j = c;
                    for (; j < c + f; j++) if (h = b[j], h.isType(this.NUMBER)) g[g.length] = h.text; else return void console.error("Parameter type is not a number: " + e + "," + h.text);
                    let i;
                    if (this.PARAMS[e]) i = {
                        key: e,
                        data: g
                    }; else return void console.error("Unsupported segment type: " + e);
                    this.segments.push(i), c += f, d = b[c], "M" == e && (e = "L"), "m" == e && (e = "l")
                } else console.error("Path data ended before all parameters were found")
            }
        }

        tokenize(a) {
            for (var b = []; "" != a;) if (a.match(/^([ \t\r\n,]+)/)) a = a.substr(RegExp.$1.length); else if (a.match(/^([aAcChHlLmMqQsStTvVzZ])/)) b[b.length] = new n(this.COMMAND, RegExp.$1), a = a.substr(RegExp.$1.length); else if (a.match(/^(([-+]?[0-9]+(\.[0-9]*)?|[-+]?\.[0-9]+)([eE][-+]?[0-9]+)?)/)) b[b.length] = new n(this.NUMBER, parseFloat(RegExp.$1)), a = a.substr(RegExp.$1.length); else return console.error("Unrecognized segment command: " + a), null;
        ...
        get linearPoints() {
            if (!this._linearPoints) {
                ...
                for (let c of this.parsed.segments) {
                    ...
                    ("m" === d || "z" === d) && (b.length && (a.push(b), b = []), "z" === d) || c.point && b.push(c.point)
                }
    ...
    class p {
        constructor(a, b, c, h, j, k) {
            ...
            let m;
            const n = this._cosPhi * (a[0] - b[0]) / 2 + this._sinPhi * (a[1] - b[1]) / 2,
                o = -this._sinPhi * (a[0] - b[0]) / 2 + this._cosPhi * (a[1] - b[1]) / 2,
                p = this._rx * this._rx * this._ry * this._ry - this._rx * this._rx * o * o - this._ry * this._ry * n * n;
            if (0 > p) {
                let a = g(1 - p / (this._rx * this._rx * this._ry * this._ry));
                this._rx = a, this._ry = a, m = 0
            } else m = (j == k ? -1 : 1) * g(p / (this._rx * this._rx * o * o + this._ry * this._ry * n * n));
            let q = m * this._rx * o / this._ry, r = -m * this._ry * n / this._rx;
            ...
            let s = this.calculateVectorAngle((n - q) / this._rx, (o - r) / this._ry, (-n - q) / this._rx, (-o - r) / this._ry);
        ...
        getNextSegment() {
            ...
            let f = d(this._theta), g = e(this._theta), h = this._theta + this._delta, i = d(h), j = e(h);
    ...
    class r {
        ...
        fit(a) {
            ...
            for (const c of this.sets) {
                let d = c.length, e = Math.floor(a * d);
                if (5 > e) {
                    if (5 >= d) continue;
                    e = 5
                }
            ...
            let c = "";
            for (const d of b) {
                for (let a, b = 0; b < d.length; b++) a = d[b], c += 0 === b ? "M" + a[0] + "," + a[1] : "L" + a[0] + "," + a[1];
                this.closed && (c += "z ")
            }
            return c
        }
        ...
        reduce(a, b) {
            ...
            for (; d.length > b;) {
                ...
                for (let h = 1; h < d.length - 1; h++) {
                    let i = this.distance(d[h - 1], d[h]), a = this.distance(d[h], d[h + 1]),
                        b = this.distance(d[h - 1], d[h + 1]), c = (i + a + b) / 2,
                        j = g(c * (c - i) * (c - a) * (c - b));
                    (0 > e || j < e) && (e = j, f = h)
                }
                if (0 < f) d.splice(f, 1); else break
            }
            return d
        }
    }
    ...
    class s {
        ...
        linearPath(a, b, c) {
            ...
            if (2 < d) {
                let e = [];
                for (let b = 0; b < d - 1; b++) e = e.concat(this._doubleLine(a[b][0], a[b][1], a[b + 1][0], a[b + 1][1], c));
                return b && (e = e.concat(this._doubleLine(a[d - 1][0], a[d - 1][1], a[0][0], a[0][1], c))), {
                    type: "path",
                    ops: e
                }
            }
        ...
        curve(a, b) {
            let c = this._curveWithOffset(a, 1 * (1 + .2 * b.roughness), b),
        ...
        ellipse(a, b, c, d, e) {
            ...
            let k = this._ellipse(g, a, b, h, j, 1, g * this._getOffset(.1, this._getOffset(.4, 1, e), e), e),
                l = this._ellipse(g, a, b, h, j, 1.5, 0, e);
            return {type: "path", ops: k.concat(l)}
        }

        arc(a, b, c, g, h, k, l, m, n) {
            let o = a, p = b, q = i(c / 2), r = i(g / 2);
            q += this._getOffset(.01 * -q, .01 * q, n), r += this._getOffset(.01 * -r, .01 * r, n);
            let s = h, t = k;
            for (; 0 > s;) s += 2 * f, t += 2 * f;
            t - s > 2 * f && (s = 0, t = 2 * f);
            let u = 2 * f / n.curveStepCount, v = j(u / 2, (t - s) / 2), w = this._arc(v, o, p, q, r, s, t, 1, n),
                x = this._arc(v, o, p, q, r, s, t, 1.5, n), y = w.concat(x);
            return l && (m ? (y = y.concat(this._doubleLine(o, p, o + q * d(s), p + r * e(s), n)), y = y.concat(this._doubleLine(o, p, o + q * d(t), p + r * e(t), n))) : (y.push({
                op: "lineTo",
                data: [o, p]
            }), y.push({op: "lineTo", data: [o + q * d(s), p + r * e(s)]}))), {type: "path", ops: y}
        }

        hachureFillArc(a, b, c, g, h, j, k) {
            let l = a, m = b, n = i(c / 2), o = i(g / 2);
            n += this._getOffset(.01 * -n, .01 * n, k), o += this._getOffset(.01 * -o, .01 * o, k);
            let p = h, q = j;
            for (; 0 > p;) p += 2 * f, q += 2 * f;
            q - p > 2 * f && (p = 0, q = 2 * f);
            let r = (q - p) / k.curveStepCount, s = [], t = [];
            for (let f = p; f <= q; f += r) s.push(l + n * d(f)), t.push(m + o * e(f));
        ...
        solidFillShape(a, b, c) {
            ...
            if (a && b && a.length && b.length && a.length === b.length) {
                ...
                const g = a.length;
                if (2 < g) {
                    d.push({op: "move", data: [a[0] + this._getOffset(-f, f, c), b[0] + this._getOffset(-f, f, c)]});
                    for (let e = 1; e < g; e++) d.push({
                        op: "lineTo",
                        data: [a[e] + this._getOffset(-f, f, c), b[e] + this._getOffset(-f, f, c)]
                    })
                }
            }
        ...
        hachureFillShape(a, c, g) {
            ...
            if (a && c && a.length && c.length) {
                let l = a[0], n = a[0], o = c[0], p = c[0];
                for (let b = 1; b < a.length; b++) l = j(l, a[b]), n = h(n, a[b]), o = j(o, c[b]), p = h(p, c[b]);
                const i = g.hachureAngle;
                let q = g.hachureGap;
                0 > q && (q = 4 * g.strokeWidth), q = h(q, .1);
                const r = i % 180 * (f / 180), s = d(r), t = e(r), u = b(r),
                    v = new m(o - 1, p + 1, l - 1, n + 1, q, t, s, u);
                for (let b; null != (b = v.getNextLine());) {
                    let d = this._getIntersectingLines(b, a, c);
                    for (let a = 0; a < d.length; a++) if (a < d.length - 1) {
                        let b = d[a], c = d[a + 1];
        ...
        hachureFillEllipse(a, c, d, e, h) {
            let j = [], k = i(d / 2), l = i(e / 2);
            k += this._getOffset(.05 * -k, .05 * k, h), l += this._getOffset(.05 * -l, .05 * l, h);
            let m = h.hachureAngle, n = h.hachureGap;
            0 >= n && (n = 4 * h.strokeWidth);
            let o = h.fillWeight;
            0 > o && (o = h.strokeWidth / 2);
            let p = b(m % 180 * (f / 180)), q = l / k, r = g(q * p * q * p + 1), s = q * p / r, t = 1 / r,
                u = n / (k * l / g(l * t * (l * t) + k * s * (k * s)) / k), v = g(k * k - (a - k + u) * (a - k + u));
            for (let w = a - k + u; w < a + k; w += u) {
                v = g(k * k - (a - w) * (a - w));
                let b = this._affine(w, c - v, a, c, s, t, q), d = this._affine(w, c + v, a, c, s, t, q);
        ...
        svgPath(a, b) {
            ...
            let c = new q(a);
            if (b.simplification) {
                let a = new r(c.linearPoints, c.closed), e = a.fit(b.simplification);
                c = new q(e)
            }
            let d = [], e = c.segments || [];
            for (let f = 0; f < e.length; f++) {
                let a = e[f], g = 0 < f ? e[f - 1] : null, h = this._processSegment(c, a, g, b);
        ...
        _bezierTo(a, b, c, d, e, g, h, j) {
            let k = [], l = [j.maxRandomnessOffset || 1, (j.maxRandomnessOffset || 1) + .5], m = null;
            for (let f = 0; 2 > f; f++) 0 === f ? k.push({op: "move", data: [h.x, h.y]}) : k.push({
                op: "move",
                data: [h.x + this._getOffset(-l[0], l[0], j), h.y + this._getOffset(-l[0], l[0], j)]
            }), m = [e + this._getOffset(-l[f], l[f], j), g + this._getOffset(-l[f], l[f], j)], k.push({
                op: "bcurveTo",
                data: [a + this._getOffset(-l[f], l[f], j), b + this._getOffset(-l[f], l[f], j), c + this._getOffset(-l[f], l[f], j), d + this._getOffset(-l[f], l[f], j), m[0], m[1]]
            });
        ...
        _processSegment(a, b, c, d) {
            ...
            switch (b.key) {
                ...
                case"m": {
                    let c = "m" === b.key;
                    if (2 <= b.data.length) {
                        let f = +b.data[0], g = +b.data[1];
                        c && (f += a.x, g += a.y);
                        let h = 1 * (d.maxRandomnessOffset || 0);
                        f += this._getOffset(-h, h, d), g += this._getOffset(-h, h, d), a.setPosition(f, g), e.push({
                            op: "move",
                            data: [f, g]
                        })
                    }
                ...
                case"l": {
                    ...
                    if (2 <= b.data.length) {
                        let f = +b.data[0], g = +b.data[1];
                        c && (f += a.x, g += a.y), e = e.concat(this._doubleLine(a.x, a.y, f, g, d)), a.setPosition(f, g)
                    }
                    break
                }
                case"H":
                case"h": {
                    ...
                    if (b.data.length) {
                        let f = +b.data[0];
                        c && (f += a.x), e = e.concat(this._doubleLine(a.x, a.y, f, a.y, d)), a.setPosition(f, a.y)
                    }
                    break
                }
                case"V":
                case"v": {
                    ...
                    if (b.data.length) {
                        let f = +b.data[0];
                        c && (f += a.y), e = e.concat(this._doubleLine(a.x, a.y, a.x, f, d)), a.setPosition(a.x, f)
                    }
                    break
                }
                case"Z":
                ...
                case"c": {
                    const c = "c" === b.key;
                    if (6 <= b.data.length) {
                        let f = +b.data[0], g = +b.data[1], h = +b.data[2], i = +b.data[3], j = +b.data[4],
                            k = +b.data[5];
                        c && (f += a.x, h += a.x, j += a.x, g += a.y, i += a.y, k += a.y);
                        let l = this._bezierTo(f, g, h, i, j, k, a, d);
                        e = e.concat(l), a.bezierReflectionPoint = [j + (j - h), k + (k - i)]
                    }
                    break
                }
                case"S":
                case"s": {
                    const f = "s" === b.key;
                    if (4 <= b.data.length) {
                        let h = +b.data[0], i = +b.data[1], j = +b.data[2], k = +b.data[3];
                        f && (h += a.x, j += a.x, i += a.y, k += a.y);
                        let l = h, m = i, n = c ? c.key : "";
                        var g = null;
                        ("c" == n || "C" == n || "s" == n || "S" == n) && (g = a.bezierReflectionPoint), g && (l = g[0], m = g[1]);
                        let o = this._bezierTo(l, m, h, i, j, k, a, d);
                        e = e.concat(o), a.bezierReflectionPoint = [j + (j - h), k + (k - i)]
                    }
                    break
                }
                case"Q":
                case"q": {
                    const c = "q" === b.key;
                    if (4 <= b.data.length) {
                        let g = +b.data[0], h = +b.data[1], i = +b.data[2], j = +b.data[3];
                        c && (g += a.x, i += a.x, h += a.y, j += a.y);
                        let k = 1 * (1 + .2 * d.roughness), l = 1.5 * (1 + .22 * d.roughness);
                        e.push({op: "move", data: [a.x + this._getOffset(-k, k, d), a.y + this._getOffset(-k, k, d)]});
                        let m = [i + this._getOffset(-k, k, d), j + this._getOffset(-k, k, d)];
                        e.push({
                            op: "qcurveTo",
                            data: [g + this._getOffset(-k, k, d), h + this._getOffset(-k, k, d), m[0], m[1]]
                        }), e.push({
                            op: "move",
                            data: [a.x + this._getOffset(-l, l, d), a.y + this._getOffset(-l, l, d)]
                        }), m = [i + this._getOffset(-l, l, d), j + this._getOffset(-l, l, d)], e.push({
                            op: "qcurveTo",
                            data: [g + this._getOffset(-l, l, d), h + this._getOffset(-l, l, d), m[0], m[1]]
                        }), a.setPosition(m[0], m[1]), a.quadReflectionPoint = [i + (i - g), j + (j - h)]
                    }
                    break
                }
                case"T":
                case"t": {
                    const h = "t" === b.key;
                    if (2 <= b.data.length) {
                        let i = +b.data[0], j = +b.data[1];
                        h && (i += a.x, j += a.y);
                        let k = i, l = j, m = c ? c.key : "";
                        var g = null;
                        ("q" == m || "Q" == m || "t" == m || "T" == m) && (g = a.quadReflectionPoint), g && (k = g[0], l = g[1]);
                        let n = 1 * (1 + .2 * d.roughness), o = 1.5 * (1 + .22 * d.roughness);
                        e.push({op: "move", data: [a.x + this._getOffset(-n, n, d), a.y + this._getOffset(-n, n, d)]});
                        let p = [i + this._getOffset(-n, n, d), j + this._getOffset(-n, n, d)];
                        e.push({
                            op: "qcurveTo",
                            data: [k + this._getOffset(-n, n, d), l + this._getOffset(-n, n, d), p[0], p[1]]
                        }), e.push({
                            op: "move",
                            data: [a.x + this._getOffset(-o, o, d), a.y + this._getOffset(-o, o, d)]
                        }), p = [i + this._getOffset(-o, o, d), j + this._getOffset(-o, o, d)], e.push({
                            op: "qcurveTo",
                            data: [k + this._getOffset(-o, o, d), l + this._getOffset(-o, o, d), p[0], p[1]]
                        }), a.setPosition(p[0], p[1]), a.quadReflectionPoint = [i + (i - k), j + (j - l)]
                    }
                    break
                }
                case"A":
                case"a": {
                    ...
                    if (7 <= b.data.length) {
                        let f = +b.data[0], g = +b.data[1], h = +b.data[2], i = +b.data[3], j = +b.data[4],
                            k = +b.data[5], l = +b.data[6];
                        if (c && (k += a.x, l += a.y), k == a.x && l == a.y) break;
                        if (0 == f || 0 == g) e = e.concat(this._doubleLine(a.x, a.y, k, l, d)), a.setPosition(k, l); else {
                            d.maxRandomnessOffset || 0;
                            for (let b = 0; 1 > b; b++) {
                                let b = new p([a.x, a.y], [k, l], [f, g], h, !!i, !!j), c = b.getNextSegment();
                                for (; c;) {
                                    let f = this._bezierTo(c.cp1[0], c.cp1[1], c.cp2[0], c.cp2[1], c.to[0], c.to[1], a, d);
        ...
        _doubleLine(a, b, c, d, e) {
            const f = this._line(a, b, c, d, e, !0, !1), g = this._line(a, b, c, d, e, !0, !0);
            return f.concat(g)
        }
        ...
        _curve(a, c, d) {
            ...
            let f = [];
            if (3 < e) {
                const g = [], b = 1 - d.curveTightness;
                f.push({op: "move", data: [a[1][0], a[1][1]]});
                for (let c = 1; c + 2 < e; c++) {
                    const d = a[c];
                    g[0] = [d[0], d[1]], g[1] = [d[0] + (b * a[c + 1][0] - b * a[c - 1][0]) / 6, d[1] + (b * a[c + 1][1] - b * a[c - 1][1]) / 6], g[2] = [a[c + 1][0] + (b * a[c][0] - b * a[c + 2][0]) / 6, a[c + 1][1] + (b * a[c][1] - b * a[c + 2][1]) / 6], g[3] = [a[c + 1][0], a[c + 1][1]], f.push({
                        op: "bcurveTo",
                ...
                if (c && 2 === c.length) {
                    ...
                    f.push({ops: "lineTo", data: [c[0] + this._getOffset(-a, a, d), c[1] + +this._getOffset(-a, a, d)]})
                }
            } else 3 === e ? (f.push({op: "move", data: [a[1][0], a[1][1]]}), f.push({
                op: "bcurveTo",
        ...
        _ellipse(a, b, c, g, h, i, j, k) {
            const l = this._getOffset(-.5, .5, k) - f / 2, m = [];
            m.push([this._getOffset(-i, i, k) + b + .9 * g * d(l - a), this._getOffset(-i, i, k) + c + .9 * h * e(l - a)]);
            for (let n = l; n < 2 * f + l - .01; n += a) m.push([this._getOffset(-i, i, k) + b + g * d(n), this._getOffset(-i, i, k) + c + h * e(n)]);
        ...
        _curveWithOffset(a, b, c) {
            const d = [[a[0][0] + this._getOffset(-b, b, c), a[0][1] + this._getOffset(-b, b, c)], [a[0][0] + this._getOffset(-b, b, c), a[0][1] + this._getOffset(-b, b, c)]];
            for (let e = 1; e < a.length; e++) d.push([a[e][0] + this._getOffset(-b, b, c), a[e][1] + this._getOffset(-b, b, c)]), e === a.length - 1 && d.push([a[e][0] + this._getOffset(-b, b, c), a[e][1] + this._getOffset(-b, b, c)]);
            return this._curve(d, null, c)
        }

        _arc(a, b, c, f, g, h, i, j, k) {
            const l = h + this._getOffset(-.1, .1, k), m = [];
            m.push([this._getOffset(-j, j, k) + b + .9 * f * d(l - a), this._getOffset(-j, j, k) + c + .9 * g * e(l - a)]);
            for (let n = l; n <= i; n += a) m.push([this._getOffset(-j, j, k) + b + f * d(n), this._getOffset(-j, j, k) + c + g * e(n)]);
            return m.push([b + f * d(i), c + g * e(i)]), m.push([b + f * d(i), c + g * e(i)]), this._curve(m, null, k)
        }

        _getIntersectingLines(b, c, d) {
            let e = [];
            const f = new l(b[0], b[1], b[2], b[3]);
            let g = 0;
            for (; g < c.length; g++) {
    ...
    class t {
        ...
        get lib() {
            if (!this._renderer) if (self && self.workly && this.config.async && !this.config.noWorker) {
                const a = Function.prototype.toString,
                    b = this.config.worklyURL || "https://cdn.jsdelivr.net/gh/pshihn/workly/dist/workly.min.js",
        ...
        rectangle(a, b, c, d, e) {
            const f = this._options(e), g = [];
            if (f.fill) {
                const e = [a, a + c, a + c, a], h = [b, b, b + d, b + d];
                "solid" === f.fillStyle ? g.push(this.lib.solidFillShape(e, h, f)) : g.push(this.lib.hachureFillShape(e, h, f))
            }
        ...
        polygon(a, b) {
            const c = this._options(b), d = [];
            if (c.fill) {
                let b = [], e = [];
                for (let c of a) b.push(c[0]), e.push(c[1]);
                "solid" === c.fillStyle ? d.push(this.lib.solidFillShape(b, e, c)) : d.push(this.lib.hachureFillShape(b, e, c))
            }
            return d.push(this.lib.linearPath(a, !0, c)), this._drawable("polygon", d, c)
        }

        arc(a, b, c, d, e, f, g, h) {
            const i = this._options(h), j = [];
        ...
        path(a, b) {
            ...
            if (c.fill) if ("solid" === c.fillStyle) {
                ...
                let d = [0, b[0], b[0], 0], f = [0, 0, b[1], b[1]], g = this.lib.hachureFillShape(d, f, c);
        ...
        _computePathSize(a) {
            ...
            if (self.document) try {
                ...
                e.setAttribute("d", a), d.appendChild(e), self.document.body.appendChild(d);
                let f = e.getBBox();
                f && (b[0] = f.width || 0, b[1] = f.height || 0), self.document.body.removeChild(d)
            } catch (a) {
            }
    ...
    class u extends t {
        ...
        async rectangle(a, b, c, d, e) {
            ...
            if (f.fill) {
                const e = [a, a + c, a + c, a], h = [b, b, b + d, b + d];
                "solid" === f.fillStyle ? g.push((await this.lib.solidFillShape(e, h, f))) : g.push((await this.lib.hachureFillShape(e, h, f)))
            }
            return g.push((await this.lib.rectangle(a, b, c, d, f))), this._drawable("rectangle", g, f)
        }
        ...
        async ellipse(a, b, c, d, e) {
            ...
            return g.push((await this.lib.ellipse(a, b, c, d, f))), this._drawable("ellipse", g, f)
        }
        ...
        async linearPath(a, b) {
            const c = this._options(b);
            return this._drawable("linearPath", [await this.lib.linearPath(a, !1, c)], c)
        }
        ...
        async polygon(a, b) {
            ...
            if (c.fill) {
                let b = [], e = [];
                for (let c of a) b.push(c[0]), e.push(c[1]);
                "solid" === c.fillStyle ? d.push((await this.lib.solidFillShape(b, e, c))) : d.push((await this.lib.hachureFillShape(b, e, c)))
            }
            return d.push((await this.lib.linearPath(a, !0, c))), this._drawable("polygon", d, c)
        }
        ...
        async arc(a, b, c, d, e, f, g, h) {
            ...
            return j.push((await this.lib.arc(a, b, c, d, e, f, g, !0, i))), this._drawable("arc", j, i)
        }
        ...
        async path(a, b) {
            ...
            if (c.fill) if ("solid" === c.fillStyle) {
                ...
                const b = this._computePathSize(a);
                let d = [0, b[0], b[0], 0], f = [0, 0, b[1], b[1]], g = await this.lib.hachureFillShape(d, f, c);
                g.type = "path2Dpattern", g.size = b, g.path = a, e.push(g)
            }
    ...
    class v {
        ...
        draw(a) {
            let b = a.sets || [], c = a.options || this.gen.defaultOptions, d = this.ctx;
            for (let e of b) switch (e.type) {
                case"path":
                    d.save(), d.strokeStyle = c.stroke, d.lineWidth = c.strokeWidth, this._drawToContext(d, e), d.restore();
                    break;
                case"fillPath":
                    d.save(), d.fillStyle = c.fill, this._drawToContext(d, e, c), d.restore();
                    break;
                case"fillSketch":
                ...
                case"path2Dpattern": {
                    let a = e.size, b = document.createElement("canvas");
                    b.width = a[0], b.height = a[1], this._fillSketch(b.getContext("2d"), e, c), this.ctx.save(), this.ctx.fillStyle = this.ctx.createPattern(b, "repeat");
...
const extend = function extend(base) {
    for (var _len = arguments.length, extensions = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
        extensions[_key - 1] = arguments[_key]
    }
    return Object.assign.apply(Object, [{}, base].concat(extensions))
};
const parseColor = function parseColor(_ref) {
    const h = _ref.h, s = _ref.s, l = _ref.l, a = _ref.a;
    return 'hsla(' + h + ',' + s + '%,' + l + '%,' + a + ')'
};
...
const colors = [Color({h: 20, s: 100, l: 50}), Color({h: 200, l: 50}), Color({h: 300, l: 50}), Color({h: 100, l: 40})];
const animationLoop = function animationLoop(scope) {
    if (scope.animation) {
    ...
    scope.particles.map(function (p, i) {
        ...
        rc.line(p.pos.x, p.pos.y + p.radius * 1.2, p.pos.x, p.pos.y + p.radius / 2, {bowing: Math.random() * 3});
